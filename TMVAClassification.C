...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   TFile *inputSig(0);
   TString fnameSig = "/Users/user/ldmx-sw/ALPSamples/FilesForBDT/Signal_PhotonFusion.root";
   inputSig = TFile::Open( fnameSig ); // check if file in local directory exists
   TFile *inputBkg(0);
   TString fnameBkg = "/Users/user/ldmx-sw/ALPSamples/FilesForBDT/Background.root";
   inputBkg = TFile::Open( fnameBkg ); // check if file in local directory exists


   // Register the training and test trees
   TTree *signalTree     = (TTree*)inputSig->Get("Features");
   TTree *background     = (TTree*)inputBkg->Get("Features");
   ...
   dataloader->AddVariable( "ZLength", 'F' );
   dataloader->AddVariable( "ZAverage", 'F' );
   dataloader->AddVariable( "ZWidth", 'F' );
   dataloader->AddVariable( "XYWidth", 'F' );
   ...
   TCut preselectionCut = " !TMath::IsNaN(NHits) && !TMath::IsNaN(ZLength) && !TMath::IsNaN(ZAverage) && !TMath::IsNaN(ZWidth) && !TMath::IsNaN(XYWidth)";
   ...
   if (Use["BDTG"]) // Gradient Boost
      factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDTG",
                           "!H:!V:NTrees=100:MinNodeSize=2.5%:BoostType=Grad:Shrinkage=0.10:UseBaggedBoost:BaggedSampleFraction=0.5:nCuts=20:MaxDepth=2" );
